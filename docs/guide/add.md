...
Input in a non-CRS84 CRS is reprojected to the boundaries' CRS (OGC:CRS84) before the intersection, so projected data joins correctly instead of erroring on a CRS mismatch.

The join predicate is a bare `ST_Intersects`, which DuckDB executes with its indexed `SPATIAL_JOIN` operator regardless of whether the input is native-geometry (GeoParquet 2.0 / geoarrow) or GeoParquet 1.x with a `bbox` column. An earlier version ANDed a per-row bbox-overlap test in front of `ST_Intersects` for 1.x inputs, but on DuckDB 1.5.1 that forces the plan to a much slower nested-loop join (verified against a remote ≥1M-feature Overture admin in [#545](https://github.com/geoparquet/geoparquet-io/issues/545)), so it was removed. A stored `bbox` covering is still used for the separate, beneficial extent pre-filter that bounds which remote boundaries are read.
