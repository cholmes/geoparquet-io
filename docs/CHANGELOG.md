...
- **An explicit `parquet-geo-only` write drops the carried `geo` key even when the geometry column is gone.** #687 fixed the geometry-present case, but the strip sat inside geometry-present guards, so a table whose geometry column had been projected away still wrote a `geo` key naming a `primary_column` the file does not contain. Auto mode (`geoparquet_version=None`) is deliberately unchanged. ([#701](https://github.com/geoparquet/geoparquet-io/issues/701), [#753](https://github.com/geoparquet/geoparquet-io/issues/753))
