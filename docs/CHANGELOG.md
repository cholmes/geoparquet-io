...
### Fixed
- **`gpio add a5` no longer intermittently exits 139 (SIGSEGV).** The `a5` community extension POSTs load-time telemetry to its vendor from a detached thread that outlives the `LOAD`; when the process exits while that thread is mid-TLS-handshake it dereferences already-freed OpenSSL locks and dies, after the output file is safely written. gpio now opts out of that telemetry before loading any community extension, unless `QUERY_FARM_TELEMETRY_OPT_OUT` is already set. Measured at 17/225 invocations before the fix and 0/150 after. ([#779](https://github.com/geoparquet/geoparquet-io/issues/779))
