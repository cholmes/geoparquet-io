...
- **`gpio process aggregate a5`** and **`gpio process aggregate admin`**: aggregate large
  GeoParquet datasets into A5 grid cells or administrative regions with per-bucket `count`,
  ...
  (`a5_cell`/`admin_code`) so `none` output can be re-joined to geometry later. Features
  outside all admin regions go into an `unassigned` bucket. Python API:
  `Table.aggregate_a5`, `Table.aggregate_admin`, `ops.aggregate_a5`, `ops.aggregate_admin`.
