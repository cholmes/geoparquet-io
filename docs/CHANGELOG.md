...
- **`gpio add country-codes` no longer exits non-zero after writing a good file** when the countries file has no subdivision column; the results summary is now built from the columns actually written, and the output path is escaped exactly once. ([#672](https://github.com/geoparquet/geoparquet-io/issues/672), [#750](https://github.com/geoparquet/geoparquet-io/issues/750))
