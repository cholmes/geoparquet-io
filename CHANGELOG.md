...
- **`gpio add s2` and `gpio partition s2` are unavailable until the `geography` extension is republished, and now say so up front.** That community extension is published only up to DuckDB 1.5.1, below gpio's new floor, because its build fails upstream on a C++11/C++17 link error (duckdb/duckdb#22097). Both commands check for it *before* reading input, draining stdin or writing anything, and stop with a message naming the command, the reason and the workaround (`uv pip install 'duckdb==1.5.1'`, which reintroduces the segfault above). The upstream fix is merged (paleolimbot/duckdb-geography#34), so S2 returns with no gpio change once the extension is rebuilt; every other command is unaffected. ([#737](https://github.com/geoparquet/geoparquet-io/issues/737))
