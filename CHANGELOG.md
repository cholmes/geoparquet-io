...
- **`gpio add bbox-metadata` is metadata-only again, and no longer invents a `geo` block.** On a 1.x WKB input it silently rewrote the geometry column as a native Parquet GEOMETRY logical type while leaving the declared version at `1.1.0` — a combination `gpio check spec` rejects; the column is now cast back to plain WKB, and a native 2.0 input still keeps its native type. On plain Parquet with a bbox column it used to report success while writing a `1.1.0` `geo` block containing only `covering` (no `encoding`, no `geometry_types`, five failed spec checks); it now fails and points at `gpio convert geoparquet`, the same way a 1.0 input is refused. ([#712](https://github.com/geoparquet/geoparquet-io/issues/712), [#713](https://github.com/geoparquet/geoparquet-io/issues/713))
