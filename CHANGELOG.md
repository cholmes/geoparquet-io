...
- **`gpio convert csv/flatgeobuf/geopackage/shapefile` now say why `-` does not work.** Reading stdin is unsupported for these four (only `convert geojson` and `convert geoparquet` consume an Arrow IPC stream); they failed with the misleading `File not found: -` and now name the limitation and give the materialize-first workaround. ([#749](https://github.com/geoparquet/geoparquet-io/issues/749), [#752](https://github.com/geoparquet/geoparquet-io/issues/752))
