...
- **GeoParquet 2.0 output no longer drops the `bbox` covering.** The fast path now carries a declared covering verbatim, keeping the native GEOMETRY type and its statistics; a covering is written only for a column gpio computed, one the input declared, or a carried conventional `bbox`, so an unrelated `tile_bounds` column can no longer become one. The same one-entry-deep merge is what keeps `gpio add h3/s2/a5/quadkey` from losing its spatial-index covering on an input that also carries a bbox column; that case is now covered by tests. ([#738](https://github.com/geoparquet/geoparquet-io/issues/738), [#744](https://github.com/geoparquet/geoparquet-io/issues/744), [#694](https://github.com/geoparquet/geoparquet-io/issues/694))
