...
- **Native geospatial statistics are read with pyarrow, not DuckDB.** DuckDB's `parquet_metadata()` reports `[0, 0, 0, 0]` on Windows for a native GEOMETRY column whose bounds pyarrow reads back correctly from the same file, so the files gpio writes there were always correct and gpio's own reader was wrong. The three getters and both validator checks now sit on one reader (`get_native_geo_stats_by_row_group`): pyarrow for local files, DuckDB for remote, and a supplied `con=` no longer drags a local read back onto the buggy path. The `win32` validator exclusions carried by #702 and #707 are gone. ([#721](https://github.com/geoparquet/geoparquet-io/issues/721), [#748](https://github.com/geoparquet/geoparquet-io/issues/748), [#770](https://github.com/geoparquet/geoparquet-io/issues/770))
