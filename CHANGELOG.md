...
- **Non-geo key/value metadata survives a write with no geometry column, and no longer costs a metadata rewrite.** `Table.write` on the default `duckdb-kv` strategy dropped `fiboa`/`vecorel`/STAC keys for tables without a geometry column, while the other three kept them; the plain-Parquet COPY paths now emit a `KV_METADATA` clause of their own. Separately, carrying those keys no longer forces the metadata-rewrite path — DuckDB accepts `KV_METADATA` alongside `GEOPARQUET_VERSION`, so a 2.0 → 2.0 copy with a sidecar payload keeps the no-rewrite fast path instead of paying for an extra scan of the geometry column. Both go through one `build_kv_metadata_clause()` helper that escapes each key and value exactly once and quotes keys, so a key containing `:` no longer breaks the COPY. ([#708](https://github.com/geoparquet/geoparquet-io/issues/708), [#709](https://github.com/geoparquet/geoparquet-io/issues/709), [#756](https://github.com/geoparquet/geoparquet-io/issues/756))
