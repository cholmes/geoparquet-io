...
- **Spatial-index golden-value tests (#664)**: `tests/test_spatial_index_golden.py`
  ...
  a fixed seven-geometry fixture — both hemispheres, the antimeridian, a point a
  hair off a cell corner, and a polygon keyed by centroid. The suite asserts that
  `add_*_table`, `partition_by_*`, `sort quadkey`, and the `gpio add h3` CLI all
  agree on those same values, so an index-registry refactor that silently moves a
  geometry into a different cell fails a test that names the index and the point.
  a5 cases carry `network` (community extension download); the rest run offline.
